                     ...
                     else
                     {
                      ...
                      for(int i=0;i<row;i++)
                        ...
                        for(int j=0;j<col;j++)
                        {
                          ...
                          symb* i_temp=gentemp(_INT);
                            symb* j_temp=gentemp(_INT);
                          emit(EQUAL,i_temp->name,tostr(i));
                          emit(EQUAL,j_temp->name,tostr(j));
                          symb* p_temp=gentemp(_INT);
                          emit(MULT, p_temp->name, i_temp->name, tostr(8*col));
                          symb* p2_temp=gentemp(_INT);
                          emit(MULT, p2_temp->name, j_temp->name, tostr(8));
                          symb* p3_temp=gentemp(_INT);
                          emit(ADD,p3_temp->name,p_temp->name,p2_temp->name);

                          symb* useles=gentemp(_DOUBLE);


                          emit(ARRR,p->name,$1->symp->name,p3_temp->name);
                      //    gentemp(_DOUBLE);
     ...
     int row,col;
       ...
       emit (MULT, res, name1, name2);
         ...
         else
         {
          ...
          string mat2=$3->symp->name;
        //  cout<<" SEG SEG "<<mat1<< " "<<mat2<<endl;
          /*string mat1=$1->symp->name;
          string mat2=$3->symp->name;*/
           //out<<"ENTERED HERe"<<mat1<<" " <<mat2<<endl;
           int row1=table->lookup(mat1)->type->row;
           int col1=  table->lookup(mat1)->type->column;
           int row2=table->lookup(mat2)->type->row;
           int col2=  table->lookup(mat2)->type->column;
           ...
           /*symb* zero=gentemp(_DOUBLE);
           init_double init_d;
           init_d.d_value = 0.000;
           init_d.name=zero->name;
           emit(EQUAL_DOUBLE,zero->name,tostr(init_d.d_value))  ;*/
                    ...
                    for (int k=0;k<col1;k++)
                          {
                            ...
                            symb* i_temp=gentemp(_INT);
                              symb* j_temp=gentemp(_INT);
                            emit(EQUAL,i_temp->name,tostr(i));
                            emit(EQUAL,j_temp->name,tostr(k));
                            symb* p_temp=gentemp(_INT);
                            emit(MULT, p_temp->name, i_temp->name, tostr(8*col1));
                            symb* p2_temp=gentemp(_INT);
                            emit(MULT, p2_temp->name, j_temp->name, tostr(8));
                            symb* p3_temp=gentemp(_INT);
                            emit(ADD,p3_temp->name,p_temp->name,p2_temp->name);
                            ...
                            symb* p=gentemp(_DOUBLE);
                            emit(ARRR,p->name,tempMat[$1->symp->name],p3_temp->name);          // p=array 1 element
                        ...
                        //    gentemp(_DOUBLE);
                          ...
                          i_temp=gentemp(_INT);
                            j_temp=gentemp(_INT);
                            emit(EQUAL,i_temp->name,tostr(k));
                            emit(EQUAL,j_temp->name,tostr(j));
                             p_temp=gentemp(_INT);
                            emit(MULT, p_temp->name, i_temp->name, tostr(8*col2));
                           p2_temp=gentemp(_INT);
                            emit(MULT, p2_temp->name, j_temp->name, tostr(8));
                             p3_temp=gentemp(_INT);
                            emit(ADD,p3_temp->name,p_temp->name,p2_temp->name);

                           useles=gentemp(_DOUBLE);
                            first=false;

                            emit(ARRR,p1->name,mat2,p3_temp->name);         //p1=array2 element

                            symb* p2=gentemp(_DOUBLE);
                            emit(MULT,p2->name,p->name,p1->name);
                            if (first)
                              emit(EQUAL,accum->name,p2->name);
                              else
                            emit(ADD,accum->name,accum->name,p2->name);

                          //  emit(ARRL,$$->symp->name,tostr(8*(col*i+j)),p2->name);
                          }
                            addMap[$$->symp->name].push_back(accum->name);
          }
         ...
         else
         {
          ...
          for(int i=0;i<row;i++)
            ...
            //  gentemp(_DOUBLE);
              ...
              symb* p=gentemp(_DOUBLE);
              emit(ARRR,p->name,tempMat[$1->symp->name],p3_temp->name);
            //  gentemp(_DOUBLE);
              symb* p1=gentemp(_DOUBLE);
            ...
            i_temp=gentemp(_INT);
             ...
             useles=gentemp(_DOUBLE);
              ...
              emit(ARRR,p1->name,tempMat[$3->symp->name],p3_temp->name);
          ...
          //    gentemp(_DOUBLE);
            ...
            i_temp=gentemp(_INT);
             ...
             useles=gentemp(_DOUBLE);
              ...
              symb* p2=gentemp(_DOUBLE);
              emit(SUB,p2->name,p->name,p1->name);
              addMap[$$->symp->name].push_back(p2->name);
            //  emit(ARRL,$$->symp->name,tostr(8*(col*i+j)),p2->name);
            }
    ...
    //  $3->symp = conv($3->symp, $1->type->cat);
       if($1->foundBracket)
       {
       ...
       else
           {
             ...
             int mapIndex=0;
    if(addMap.find($3->symp->name)!=addMap.end())
            for(int i=0;i<row;i++)
            ...
            //    gentemp(_DOUBLE);
              ...
              symb* useles=gentemp(_DOUBLE);
                ...
                emit(ARRL,$1->symp->name,p3_temp->name,p->name);
              }
              else
              {
                /*for(int i=0;i<row;i++)
                  ...
                  for(int j=0;j<col;j++)
                        {
                          ...
                          symb* p3_temp=gentemp(_INT);
                          emit(ADD,p3_temp->name,p_temp->name,p2_temp->name);
                          symb* hey=gentemp(_DOUBLE);
                          emit(ARRR,hey->name,tempMat[$3->symp->name],p3_temp->name);

                          emit(ARRL,$1->symp->name,p3_temp->name,hey->name);
                          //emit(EQUAL,$1->symp->name,$3->symp->name);
                        }*/
                        emit(EQUAL,$1->symp->name,tempMat[$3->symp->name]);
              }
           }
   }
...
identifier_list
 ...
 ;
                                    ...
                                    /* The following parsing rules come into play while d"eclaring a Matrix*/
